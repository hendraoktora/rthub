import { PrismaClient, Role, StatusTagihan, PaymentMethod, PaymentStatus, TipeKas, ScopeWilayah, StatusLaporan } from '@prisma/client';
...
async function main() {
  ...
  // 5. Master Tagihan Default RT 03
  ...
  // 6. Data 12 Warga & KK Lengkap dengan Anggota Keluarga
  const wargaData = [
  ...
  for (const w of wargaData) {
    ...
    // C. Buat Kartu Keluarga & Anggota Keluarga
    ...
    if (!existingTagihan) {
      await prisma.tagihanWarga.create({
        data: {
          ...
          adminFee: 2000.00,
          totalBayar: 52000.00,
          status: w.statusTagihan === 'LUNAS' ? StatusTagihan.PAID : StatusTagihan.UNPAID,
